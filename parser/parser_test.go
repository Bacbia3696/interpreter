...
func TestParsingInfixExpressions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		checkParserErrors(t, p)
		if len(program.Statements) != 1 {
			t.Fatalf("invalid # of statements: %d", len(program.Statements))
		}
		stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
		if !ok {
			t.Fatalf("program.Statements[0] is not ast.ExpressionStatement: %T", program.Statements[0])
		}
		exp, ok := stmt.Expression.(*ast.InfixExpression)
		if !ok {
			t.Fatalf("stmt.Expression is not ast.InfixExpression: %T", stmt.Expression)
		}
